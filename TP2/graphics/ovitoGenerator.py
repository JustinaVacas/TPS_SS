...
def export_to_ovito(frame_file):
    ...
    export_file(pipeline, 'results_ovito3_3.dump', 'lammps/dump',
...
def get_particles(data_frame):
    ...
    particles.create_property("Force", data=np.array((np.cos(data_frame.angle) * data_frame.speed, np.sin(data_frame.angle) * data_frame.speed, data_frame.z)).T)
